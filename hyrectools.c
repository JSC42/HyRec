...
double **create_2D_array(unsigned n1, unsigned n2, int *error, char error_message[SIZE_ErrorM])
{
  ...
  for (i = 0; i < n1; i++)
    matrix[i] = create_1D_array(n2, error, error_message);
...
double ***create_3D_array(unsigned n1, unsigned n2, unsigned n3, int *error, char error_message[SIZE_ErrorM])
{
  ...
  for (i = 0; i < n1; i++)
    matrix[i] = create_2D_array(n2, n3, error, error_message);
                    ...
                    unsigned int Nx1, unsigned int Nx2, double x1, double x2, int *error, char error_message[SIZE_ErrorM])
{
  ...
  for (j = 0; j < 4; j++)
    temp[j] = rec_interp1d(x20, dx2, ytab[j], Nx2, x2, error, error_message);
...
double rec_interpol_G(double x, double *xtab, double *ytab, unsigned int Nx, int *error, char error_message[SIZE_ErrorM])
{
  ...
  /* Return value */
  return (
      ytab[0] * (x - xtab[1]) * (x - xtab[2]) * (x - xtab[3]) / (xtab[0] - xtab[1]) / (xtab[0] - xtab[2]) / (xtab[0] - xtab[3]) + ytab[1] * (x - xtab[0]) * (x - xtab[2]) * (x - xtab[3]) / (xtab[1] - xtab[0]) / (xtab[1] - xtab[2]) / (xtab[1] - xtab[3]) + ytab[2] * (x - xtab[0]) * (x - xtab[1]) * (x - xtab[3]) / (xtab[2] - xtab[0]) / (xtab[2] - xtab[1]) / (xtab[2] - xtab[3]) + ytab[3] * (x - xtab[0]) * (x - xtab[1]) * (x - xtab[2]) / (xtab[3] - xtab[0]) / (xtab[3] - xtab[1]) / (xtab[3] - xtab[2]));
}
