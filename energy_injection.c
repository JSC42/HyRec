...
double eps_over_mdot_pbh(double Mpbh, double z, double xe, double Teff)
{
  ...
  if (X < 1)
    G = 4. / M_PI * sqrt(2. / M_PI / X) * (1. + 5.5 * pow(X, 1.25));
  else
    G = 27. / 2. / M_PI * (log(2. * X * 0.56146 + 0.08) + 4. / 3.);
...
{
  return dEdtdV_DM_ann(z, params) + dEdtdV_pbh(params->fpbh, params->Mpbh, z, xe, Tgas);
}
                       ...
                       double nH, double H, INJ_PARAMS *params, double *dEdtdV_dep)
{
  ...
  { // c sigma_T = 2e-14 (cgs)
    *dEdtdV_dep = (*dEdtdV_dep * exp(-7. * dlna) + 2e-15 * dlna * nH / H * inj) / (1. + 2e-15 * dlna * nH / H);
  }
}
