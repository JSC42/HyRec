...
void rec_get_cosmoparam(FILE *fin, FILE *fout, REC_COSMOPARAMS *param)
{
  ...
  if (fout != NULL)
    fprintf(fout, "ann_z: \n");
  if (fscanf(fin, "%lg", &(param->inj_params->ann_z)) != 1)
  {
    if (fout != NULL)
      fprintf(fout, "Error in rec_get_cosmoparam when reading parameter 'ann_z'\n");
    exit(1);
  };
  if (fout != NULL)
    fprintf(fout, "ann_zmax: \n");
  if (fscanf(fin, "%lg", &(param->inj_params->ann_zmax)) != 1)
  {
    if (fout != NULL)
      fprintf(fout, "Error in rec_get_cosmoparam when reading parameter 'ann_zmax'\n");
    exit(1);
  };
  if (fout != NULL)
    fprintf(fout, "ann_zmin: \n");
  if (fscanf(fin, "%lg", &(param->inj_params->ann_zmin)) != 1)
  {
    if (fout != NULL)
      fprintf(fout, "Error in rec_get_cosmoparam when reading parameter 'ann_zmin'\n");
    exit(1);
  };
  if (fout != NULL)
    fprintf(fout, "ann_var: \n");
  if (fscanf(fin, "%lg", &(param->inj_params->ann_var)) != 1)
  {
    if (fout != NULL)
